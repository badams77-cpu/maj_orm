...
public class DbBeanGenericInstance<T extends BaseDistillerEntity> implements DbBeanGenericInterface<T> {
...
// Cassandra Session
  ...
  private String table;
    ...
    public static class BeanTyper<T extends BaseDistillerEntity> {

        public  synchronized <T  extends BaseDistillerEntity> DbBeanGenericInterface<? extends BaseDistillerEntity> getTypedBean(DbBean dbBean, DbBeanInstance mainBean, List<T> S, Class S1){
  ...
  protected DbBeanGenericInstance(DbBean dbBean, Class<T> clazz){
      ...
      try {
        if (clazz.newInstance() instanceof BaseDistillerEntity) {

            table = ((BaseDistillerEntity) clazz.newInstance()).getTableName();
            clazzArray = (T[]) new LinkedList<T>().toArray();
        }
      } catch (Exception e){
              LOGGER.warn(" "+clazz+" does not subclass BaseDistillerEntity");
