...
//    private YamlDBConfig ydb;
    ...
    private  Map<MajDataSourceName, CqlSession> smokCassMap;
    ...
    public DBEnvSetup(CassandraState cassandraState, Map<String, String> addProp){
        ...
        Set<String> names =  dbEnvMap.keySet().stream().filter(x->x.indexOf(SEPERATOR)>0).map(x->x.substring(0,x.indexOf(SEPERATOR))).collect(Collectors.toSet());
